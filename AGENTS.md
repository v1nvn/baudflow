...
- `mix precommit` — the done-gate (warnings-as-errors, format, test). Never format or lint by hand.
- `mix check` — full gate with throwaway Postgres (testcontainers). `mix lint` is the no-DB static gate.
- `mix test test/path.exs` to target, `mix test --failed` to rerun failures.
